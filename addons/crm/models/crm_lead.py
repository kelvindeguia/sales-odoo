...
class Lead(models.Model):
    ...
    tag_ids = fields.Many2many(
        ...
        index='trigram')
    tag_it_ids = fields.Many2many(
        'crm.tag.it', 'crm_tag_it_rel', 'lead_id', 'tag_id', string='Tags IT',
        help="Classify and analyze your lead/opportunity categories like: Training, Service", store=True,
        index='trigram')
    ...
    x_remote_role = fields.Text('Potential Opportunities', store=True)
    ...
    x_dn1_ext = fields.Char('DN1 Ext.', store=True, size=10)
    x_dn2_ext = fields.Char('DN2 Ext.', store=True, size=10)
    x_cp1_ext = fields.Char('CP1 Ext.', store=True, size=10)
    x_cp2_ext = fields.Char('CP2 Ext.', store=True, size=10)
    x_neverbounce_result = fields.Char('Neverbounce Result', store=True)
    x_time_zone = fields.Char('Time Zone', store=True)
    x_candidate_endorsement = fields.Html('Candidate Endorsement', store=True)
    x_additional_contacts = fields.Text('Additional Contacts', store=True)
    x_presentation_date = fields.Datetime('Show Date', store=True, readonly=False)
    x_client_headcount = fields.Char('Client Headcount ', store=True)
    ...
    x_leads_sent_engagement_1_2024 = fields.Boolean('Marketing Re-Engagement #1 - Sent', store=True)
    x_leads_opened_engagement_1_2024 = fields.Boolean('Marketing Re-Engagement #1 - Opened', store=True)

    x_leads_sent_engagement_2_2024 = fields.Boolean('Marketing Re-Engagement #2 - Sent', store=True)
    x_leads_opened_engagement_2_2024 = fields.Boolean('Marketing Re-Engagement #2 - Opened', store=True)

    x_leads_sent_engagement_3_2024 = fields.Boolean('Marketing Re-Engagement #3 - Sent', store=True)
    x_leads_opened_engagement_3_2024 = fields.Boolean('Marketing Re-Engagement #3 - Opened', store=True)

    # Kathryn 500M Campaign

    x_leads_sent_email_1_2024 = fields.Boolean('Kathryn Email Blast #1 - Sent', store=True)
    x_leads_opened_email_1_2024 = fields.Boolean('Kathryn Email Blast #1 - Opened', store=True)

    x_leads_sent_email_2_2024 = fields.Boolean('Kathryn Email Blast #2 - Sent', store=True)
    x_leads_opened_email_2_2024 = fields.Boolean('Kathryn Email Blast #2 - Opened', store=True)
    
    x_leads_sent_email_3_2024 = fields.Boolean('Kathryn Email Blast #3 - Sent', store=True)
    x_leads_opened_email_3_2024 = fields.Boolean('Kathryn Email Blast #3 - Opened', store=True)

    x_leads_sent_email_4_2024 = fields.Boolean('Kathryn Email Blast #4 - Sent', store=True)
    x_leads_opened_email_4_2024 = fields.Boolean('Kathryn Email Blast #4 - Opened', store=True)

    x_leads_sent_email_5_2024 = fields.Boolean('Kathryn Email Blast #5 - Sent', store=True)
    x_leads_opened_email_5_2024 = fields.Boolean('Kathryn Email Blast #5 - Opened', store=True)

    x_leads_sent_email_6_2024 = fields.Boolean('Kathryn Email Blast #6 - Sent', store=True)
    x_leads_opened_email_6_2024 = fields.Boolean('Kathryn Email Blast #6 - Opened', store=True)

    x_leads_sent_email_7_2024 = fields.Boolean('Kathryn Email Blast #7 - Sent', store=True)
    x_leads_opened_email_7_2024 = fields.Boolean('Kathryn Email Blast #7 - Opened', store=True)

    x_leads_sent_email_8_2024 = fields.Boolean('Kathryn Email Blast #8 - Sent', store=True)
    x_leads_opened_email_8_2024 = fields.Boolean('Kathryn Email Blast #8 - Opened', store=True)

    x_leads_sent_email_9_2024 = fields.Boolean('Kathryn Email Blast #9 - Sent', store=True)
    x_leads_opened_email_9_2024 = fields.Boolean('Kathryn Email Blast #9 - Opened', store=True)

    x_leads_sent_email_10_2024 = fields.Boolean('Kathryn Email Blast #10 - Sent', store=True)
    x_leads_opened_email_10_2024 = fields.Boolean('Kathryn Email Blast #10 - Opened', store=True)

    x_leads_sent_email_11_2024 = fields.Boolean('Kathryn Email Blast #11 - Sent', store=True)
    x_leads_opened_email_11_2024 = fields.Boolean('Kathryn Email Blast #11 - Opened', store=True)

    # Kathryn Big Fish Campaign

    x_leads_sent_email_12_2024 = fields.Boolean('Kathryn Email Blast #12 - Sent', store=True)
    x_leads_opened_email_12_2024 = fields.Boolean('Kathryn Email Blast #12 - Opened', store=True)
 
    
    x_leads_sent_email_13_2024 = fields.Boolean('Kathryn Email Blast #13 - Sent', store=True)
    x_leads_opened_email_13_2024 = fields.Boolean('Kathryn Email Blast #13 - Opened', store=True)
...
#personal email
    x_personal_email = fields.Char('Personal Email', store=True)

    # start of logs

    x_log_ids = fields.Many2many('log.model', string='Change Logs', readonly=True, store=True)
    x_update_logs = fields.Text(string="Update Logs", store=True)
    ...
    def _onchange_tag_ids(self):
        ...
        if self._origin:
            ...
            # Tag IDs (Many2many field)
            old_tags = self._origin.tag_ids
            new_tags = self.tag_ids

            if old_tags != new_tags:
                old_tag_names = ', '.join(old_tags.mapped('name'))
                new_tag_names = ', '.join(new_tags.mapped('name'))
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name} | Tags: {old_tag_names} to {new_tag_names}\n"

            user_time_naive = user_time.replace(tzinfo=None)

            # Create a log entry record in the log.model
            if log_entry:
                log_data = {
                    'x_log_name': self.tag_ids,  # Adjust the log name as needed
                    'x_log_record_name': self.name,  # Adjust the record name as needed
                    'x_log_old_value': old_tag_names,
                    'x_log_new_value': new_tag_names,
                    'x_log_date': user_time_naive,
                    'x_log_user_id': self.env.user.id,
                    'x_log_value': self.id,
                    'x_log_document_model': self._name,
                    'x_log_field_change': 'Tags',  # Set the field change to 'tag_ids'
                }
                self.env['log.model'].create(log_data)

            # Update the log field
            if log_entry:
                self.x_update_logs = (self.x_update_logs or "") + log_entry

    # add opt out email DNE
    def add_opt_out_email(self):
        tag_model = self.env['crm.tag']
        opt_out_tag = tag_model.search([('name', '=', 'opt-out email')], limit=1)
        if opt_out_tag:
            old_tags = self.tag_ids.ids  # Get the current tag IDs before adding
            self.write({'tag_ids': [(4, opt_out_tag.id)]})
            new_tags = self.tag_ids.ids  # Get the updated tag IDs after adding
            self.log_tag_change('opt-out email', old_tags, new_tags)  # Log the tag change


        # add verified V

    def add_verified_email(self):
        tag_model = self.env['crm.tag']
        opt_out_tag = tag_model.search([('name', '=', 'Verified')], limit=1)
        if opt_out_tag:
            old_tags = self.tag_ids.ids  # Get the current tag IDs before adding
            self.write({'tag_ids': [(4, opt_out_tag.id)]})
            new_tags = self.tag_ids.ids  # Get the updated tag IDs after adding
            self.log_tag_change('Verified', old_tags, new_tags)  # Log the tag change

    
    def button_click_verified(self):
        self.add_verified_email()


    

    def log_tag_change(self, tag_name, old_tags, new_tags):
        user_timezone = 'Asia/Singapore'
        utc_now = datetime.utcnow()
        # Convert time
        user_timezone = pytz.timezone(user_timezone)
        ...
        # Create a log entry for the added tag
        log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name} | DNE | Tag Added:  {tag_name}\n"

        # Convert tag IDs to tag names for old and new values
        old_tag_names = ', '.join(self.env['crm.tag'].browse(old_tags).mapped('name'))
        new_tag_names = ', '.join(self.env['crm.tag'].browse(new_tags).mapped('name'))

        log_entry += f"Old Tags: {old_tag_names}\n"
        log_entry += f"New Tags: {new_tag_names}\n"

        user_time_naive = user_time.replace(tzinfo=None)

        # Set x_log_value to the ID of the current record
        x_log_value = self.id

        # Create a log entry record in the log.model
        if log_entry:
            log_data = {
                'x_log_name': tag_name,  # Adjust the log name as needed
                'x_log_record_name': self.name,  # Adjust the record name as needed
                'x_log_old_value': old_tag_names,
                'x_log_new_value': new_tag_names,
                'x_log_date': user_time_naive,
                'x_log_user_id': self.env.user.id,
                'x_log_value': x_log_value,  # Set x_log_value explicitly to the ID of the current record
                'x_log_document_model': self._name,
                'x_log_field_change': 'DNE',  # Set the field change to 'tag_ids'
            }
            log_entry_record = self.env['log.model'].create(log_data)

            # Update the x_log_ids field with the created log entry record
            self.x_log_ids = [(4, log_entry_record.id)]
    ...
    def add_opt_out_call(self):
        ...
        if opt_out_tag:
            ...
            self.write({'tag_ids': [(4, opt_out_tag.id)]})
            new_tags = self.tag_ids.ids  # Get the updated tag IDs after adding
            self.log_tag_change_call('opt-out call', old_tags, new_tags)  # Log the tag change

    def log_tag_change_call(self, tag_name, old_tags, new_tags):
        user_timezone = 'Asia/Singapore'
        utc_now = datetime.utcnow()
        # Convert time
        user_timezone = pytz.timezone(user_timezone)
        ...
        # Create a log entry for the added tag
        log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name} | DNC | Tag Added: {tag_name}\n"
        ...
        # Set x_log_value to self.id explicitly
        x_log_value = self.id

        # Create a log entry record in the 
        if log_entry:
            log_data = {
                'x_log_name': tag_name,  # Adjust the log name as needed
                'x_log_record_name': self.name,  # Adjust the record name as needed
                ...
                'x_log_value': x_log_value,  # Set x_log_value explicitly to the ID of the current record
                'x_log_document_model': self._name,
                'x_log_field_change': 'DNC',  # Set the field change to 'tag_ids'
            }
            log_entry_record = self.env['log.model'].create(log_data)
    ...
    @api.onchange('email_from', 'name', 'website', 'x_address', 'x_company_linkedin', 'x_remote_role', 'contact_name',
                  'function', 'x_personal_linkedin', 'type', 'user_id', 'team_id', 'partner_name', 'x_direct_number_1',
                  'x_direct_number_2', 'phone', 'x_company_phone_2', 'x_call_disposition_dn1_id',
                  'x_call_disposition_dn2_id', 'x_call_disposition_cp1_id', 'x_call_disposition_cp2_id')
    def _onchange_fields(self):
        ...
        if self._origin:
            ...
            if self.name != self._origin.name:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Name: {self._origin.name} to {self.name}\n"
            ...
            if log_entry:
                log_data = {
                    'x_log_name': self.name,
                    'x_log_record_name': self.name,
                    'x_log_old_value': self._origin.name,
                    'x_log_new_value': self.name,
                    'x_log_date': user_time_naive,
                    'x_log_user_id': self.env.user.id,
                    'x_log_value': self.id,
                    'x_log_document_model': self._name,
                    'x_log_field_change': 'Name',
                }
                self.env['log.model'].create(log_data)

            # # Tags
            # added_tags = self.tag_ids - self._origin.tag_ids
            # removed_tags = self._origin.tag_ids - self.tag_ids
            #
            # if added_tags:
            #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name} | Tags added: {', '.join(added_tags.mapped('name'))}\n"
            # if removed_tags:
            #     log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name} | Tags removed: {', '.join(removed_tags.mapped('name'))}\n"

            # Email
            if self.email_from != self._origin.email_from:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Email: {self._origin.email_from} to {self.email_from}\n"
            ...
            if log_entry:
                log_data = {
                    'x_log_name': self.email_from,
                    'x_log_record_name': self.name,
                    'x_log_old_value': self._origin.email_from,
                    'x_log_new_value': self.email_from,
                    'x_log_date': user_time_naive,
                    'x_log_user_id': self.env.user.id,
                    'x_log_value': self.id,
                    'x_log_document_model': self._name,
                    'x_log_field_change': 'Email',
                }
                self.env['log.model'].create(log_data)

            # Company Information
            if self.partner_name != self._origin.partner_name:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Company Name: {self._origin.partner_name} to {self.partner_name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.partner_name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.partner_name,
                        'x_log_new_value': self.partner_name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Company Name',
                    }
                    self.env['log.model'].create(log_data)

            if self.website != self._origin.website:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Website: {self._origin.website} to {self.website}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.website,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.website,
                        'x_log_new_value': self.website,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Website',
                    }
                    self.env['log.model'].create(log_data)

            if self.x_address != self._origin.x_address:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Address: {self._origin.x_address} to {self.x_address}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_address,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_address,
                        'x_log_new_value': self.x_address,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Address',
                    }
                    self.env['log.model'].create(log_data)

            if self.x_company_linkedin != self._origin.x_company_linkedin:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Company LinkedIn: {self._origin.x_company_linkedin} to {self.x_company_linkedin}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_company_linkedin,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_company_linkedin,
                        'x_log_new_value': self.x_company_linkedin,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Company LinkedIn',
                    }
                    self.env['log.model'].create(log_data)

            if self.x_remote_role != self._origin.x_remote_role:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Remote Role: {self._origin.x_remote_role} to {self.x_remote_role}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_remote_role,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_remote_role,
                        'x_log_new_value': self.x_remote_role,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Remote Role',
                    }
                    self.env['log.model'].create(log_data)

            # Personal Information
            if self.contact_name != self._origin.contact_name:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Contact Name: {self._origin.contact_name} to {self.contact_name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.contact_name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.contact_name,
                        'x_log_new_value': self.contact_name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Contact Name',
                    }
                    self.env['log.model'].create(log_data)

            if self.function != self._origin.function:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Job Position: {self._origin.function} to {self.function}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.function,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.function,
                        'x_log_new_value': self.function,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Job Position',
                    }
                    self.env['log.model'].create(log_data)

            if self.x_personal_linkedin != self._origin.x_personal_linkedin:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Personal Linkedin: {self._origin.x_personal_linkedin} to {self.x_personal_linkedin}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_personal_linkedin,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_personal_linkedin,
                        'x_log_new_value': self.x_personal_linkedin,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Personal Linkedin',
                    }
                    self.env['log.model'].create(log_data)

            # Sales Information
            if self.type != self._origin.type:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Type:  {self._origin.type} to {self.type}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.type,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.type,
                        'x_log_new_value': self.type,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Type',
                    }
                    self.env['log.model'].create(log_data)

            if self.user_id.name != self._origin.user_id.name:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Salesperson: {self._origin.user_id.name} to {self.user_id.name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.user_id.name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.user_id.name,
                        'x_log_new_value': self.user_id.name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Salesperson',
                    }
                    self.env['log.model'].create(log_data)

            if self.team_id.name != self._origin.team_id.name:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Sales Team: {self._origin.team_id.name} to {self.team_id.name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.team_id.name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.team_id.name,
                        'x_log_new_value': self.team_id.name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Sales Team',
                    }
                    self.env['log.model'].create(log_data)

            # Contact Information
            if self.x_direct_number_1 != self._origin.x_direct_number_1:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Direct Number 1: {self._origin.x_direct_number_1} to {self.x_direct_number_1}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_direct_number_1,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_direct_number_1,
                        'x_log_new_value': self.x_direct_number_1,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Direct Number 1',
                    }
                    self.env['log.model'].create(log_data)

            if self.x_direct_number_2 != self._origin.x_direct_number_2:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Direct Number 2: {self._origin.x_direct_number_2} to {self.x_direct_number_2}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_direct_number_2,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_direct_number_2,
                        'x_log_new_value': self.x_direct_number_2,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Direct Number 2',
                    }
                    self.env['log.model'].create(log_data)

            if self.phone != self._origin.phone:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Company Phone 1: {self._origin.phone} to {self.phone}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.phone,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.phone,
                        'x_log_new_value': self.phone,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Company Phone 1',
                    }
                    self.env['log.model'].create(log_data)

            if self.x_company_phone_2 != self._origin.x_company_phone_2:
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | Company Phone 2: {self._origin.x_company_phone_2} to {self.x_company_phone_2}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_company_phone_2,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_company_phone_2,
                        'x_log_new_value': self.x_company_phone_2,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Company Phone 2',
                    }
                    self.env['log.model'].create(log_data)

            desired_ids = [22, 23, 24, 25, 26, 27, 28, 29, 63, 66, 67]
            booked_ids = [22, 67]
            if self.x_call_disposition_dn1_id.call_disposition_name != self._origin.x_call_disposition_dn1_id.call_disposition_name:
                dn1_value = self.x_direct_number_1
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | {dn1_value} | Call Disposition: {self._origin.x_call_disposition_dn1_id.call_disposition_name} to {self.x_call_disposition_dn1_id.call_disposition_name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_call_disposition_dn1_id.call_disposition_name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_call_disposition_dn1_id.call_disposition_name,
                        'x_log_new_value': self.x_call_disposition_dn1_id.call_disposition_name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                        'x_log_field_change': 'Call Disposition',
                    }
                    self.env['log.model'].create(log_data)
                    # Check if the current call disposition ID is in the desired_ids list
                    if self.x_call_disposition_dn1_id.id in desired_ids:
                        # Create a record in the 'crm.call.tracker' model
                        call_tracker_values = {
                            'call_tracker_name': self.name,
                            'call_tracker_number': self.x_direct_number_1,
                            # You may need to adjust this based on your requirements
                            'call_tracker_date': fields.Date.today(),
                            'call_tracker_state': self.x_sa_company_state,
                            'call_tracker_city': self.x_sa_company_city,
                            'call_tracker_industry': self.x_sa_industries,
                            'call_tracker_disposition': self.x_call_disposition_dn1_id.id,
                            'call_tracker_related_id': self.id,
                            'call_tracker_appointments_booked': self.x_call_disposition_dn1_id.id in booked_ids,
                            # Add other fields as needed
                        }
                        self.env['crm.call.tracker'].create(call_tracker_values)


            if self.x_call_disposition_dn2_id.call_disposition_name != self._origin.x_call_disposition_dn2_id.call_disposition_name:
                dn2_value = self.x_direct_number_2
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | {dn2_value} | Call Disposition: {self._origin.x_call_disposition_dn2_id.call_disposition_name} to {self.x_call_disposition_dn2_id.call_disposition_name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_call_disposition_dn2_id.call_disposition_name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_call_disposition_dn2_id.call_disposition_name,
                        'x_log_new_value': self.x_call_disposition_dn2_id.call_disposition_name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                    ...
                    self.env['log.model'].create(log_data)
                    # Check if the current call disposition ID is in the desired_ids list
                    if self.x_call_disposition_dn2_id.id in desired_ids:
                        # Create a record in the 'crm.call.tracker' model
                        call_tracker_values = {
                            'call_tracker_name': self.name,
                            'call_tracker_number': self.x_direct_number_2,
                            # You may need to adjust this based on your requirements
                            'call_tracker_date': fields.Date.today(),
                            'call_tracker_state': self.x_sa_company_state,
                            'call_tracker_city': self.x_sa_company_city,
                            'call_tracker_industry': self.x_sa_industries,
                            'call_tracker_disposition': self.x_call_disposition_dn2_id.id,
                            'call_tracker_related_id': self.id,
                            'call_tracker_appointments_booked': self.x_call_disposition_dn2_id.id in booked_ids,
                            # Add other fields as needed
                        }
                        self.env['crm.call.tracker'].create(call_tracker_values)

            if self.x_call_disposition_cp1_id.call_disposition_name != self._origin.x_call_disposition_cp1_id.call_disposition_name:
                cp1_value = self.phone
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | {cp1_value} | Call Disposition: {self._origin.x_call_disposition_cp1_id.call_disposition_name} to {self.x_call_disposition_cp1_id.call_disposition_name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_call_disposition_cp1_id.call_disposition_name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_call_disposition_cp1_id.call_disposition_name,
                        'x_log_new_value': self.x_call_disposition_cp1_id.call_disposition_name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                    ...
                    self.env['log.model'].create(log_data)
                    # Check if the current call disposition ID is in the desired_ids list
                    if self.x_call_disposition_cp1_id.id in desired_ids:
                        # Create a record in the 'crm.call.tracker' model
                        call_tracker_values = {
                            'call_tracker_name': self.name,
                            'call_tracker_number': self.phone,
                            # You may need to adjust this based on your requirements
                            'call_tracker_date': fields.Date.today(),
                            'call_tracker_state': self.x_sa_company_state,
                            'call_tracker_city': self.x_sa_company_city,
                            'call_tracker_industry': self.x_sa_industries,
                            'call_tracker_disposition': self.x_call_disposition_cp1_id.id,
                            'call_tracker_related_id': self.id,
                            'call_tracker_appointments_booked': self.x_call_disposition_cp1_id.id in booked_ids,
                            # Add other fields as needed
                        }
                        self.env['crm.call.tracker'].create(call_tracker_values)

            if self.x_call_disposition_cp2_id.call_disposition_name != self._origin.x_call_disposition_cp2_id.call_disposition_name:
                cp2_value = self.x_company_phone_2
                log_entry += f"{user_time:%m-%d-%Y %I:%M%p} | {user_name}  | {self.x_company_phone_2} | Call Disposition: {self._origin.x_call_disposition_cp2_id.call_disposition_name} to {self.x_call_disposition_cp2_id.call_disposition_name}\n"
                ...
                if log_entry:
                    log_data = {
                        'x_log_name': self.x_call_disposition_cp2_id.call_disposition_name,
                        'x_log_record_name': self.name,
                        'x_log_old_value': self._origin.x_call_disposition_cp2_id.call_disposition_name,
                        'x_log_new_value': self.x_call_disposition_cp2_id.call_disposition_name,
                        'x_log_date': user_time_naive,
                        'x_log_user_id': self.env.user.id,
                        'x_log_value': self.id,
                        'x_log_document_model': self._name,
                    ...
                    self.env['log.model'].create(log_data)
                    # Check if the current call disposition ID is in the desired_ids list
                    if self.x_call_disposition_cp2_id.id in desired_ids:
                        # Create a record in the 'crm.call.tracker' model
                        call_tracker_values = {
                            'call_tracker_name': self.name,
                            'call_tracker_number': self.x_company_phone_2,
                            # You may need to adjust this based on your requirements
                            'call_tracker_date': fields.Date.today(),
                            'call_tracker_state': self.x_sa_company_state,
                            'call_tracker_city': self.x_sa_company_city,
                            'call_tracker_industry': self.x_sa_industries,
                            'call_tracker_disposition': self.x_call_disposition_cp2_id.id,
                            'call_tracker_related_id': self.id,
                            'call_tracker_appointments_booked': self.x_call_disposition_cp2_id.id in booked_ids,
    ...
    # start of disposition
    x_call_disposition_dn1_id = fields.Many2one('crm.call.disposition', string='Direct Number 1 Call Disposition', store=True,
                                                )
    x_call_disposition_dn2_id = fields.Many2one('crm.call.disposition', string='Direct Number 2 Call Disposition', store=True,
                                                )
    x_call_disposition_cp1_id = fields.Many2one('crm.call.disposition', string='Company Phone 1 Call Disposition', store=True,
                                                )
    x_call_disposition_cp2_id = fields.Many2one('crm.call.disposition', string='Company Phone 2 Call Disposition', store=True


                                           )

    x_is_connected_dn1 = fields.Boolean('is connected?', store=True)
    x_is_connected_dn2 = fields.Boolean('is connected?', store=True)
    x_is_connected_cp1 = fields.Boolean('is connected?', store=True)
    x_is_connected_cp2 = fields.Boolean('is connected?', store=True)

    # Start of disposition report

    x_call_tracker_id = fields.Many2many('crm.call.tracker', string='Disposition Report', readonly=True)

    #Start of LG Shows Menu

    x_is_connected_lg_shows = fields.Boolean('is Lg Shows?', store=True, default=False)

    @api.onchange('x_is_connected_lg_shows')
    def _onchange_x_is_connected_lg_shows(self):
        for record in self:
            if record.x_is_connected_lg_shows:
                if record.x_is_connected_lg_shows == True:
                    # record.stage_id = 16
                    record.type = 'opportunity'


# disposition model
class Call_Tracker(models.Model):
    _name = "crm.call.tracker"
    _description = "Call Tracker"
    _rec_name = 'call_tracker_name'

    call_tracker_name = fields.Char(string='Name', required=True, store=True)
    call_tracker_number = fields.Char(string='Number', store=True)
    call_tracker_date = fields.Date(string='Date', store=True)
    call_tracker_state = fields.Char(string='State', store=True)
    call_tracker_city = fields.Char(string='City', store=True)
    call_tracker_industry = fields.Char(string='Industry', store=True)
    call_tracker_calls_connected = fields.Boolean(string='Calls Connected', store=True,
                                                  related='call_tracker_disposition.call_disposition_is_connected')
    call_tracker_disposition = fields.Many2one('crm.call.disposition', string='Disposition', store=True)
    call_tracker_appointments_booked = fields.Boolean(string='Appointments Booked', store=True)
    call_tracker_lead_id = fields.Many2one('crm.lead', string='Related Lead', store=True)
    call_tracker_related_id = fields.Char('Related ID_1', store=True)
    call_tracker_related_id_fix = fields.Integer('Related ID', store=True,
                                                 compute='_compute_call_tracker_related_id_fix')

    @api.depends('call_tracker_related_id')
    def _compute_call_tracker_related_id_fix(self):
        for record in self:
            if record.call_tracker_related_id and record.call_tracker_related_id.startswith('NewId_'):
                try:
                    integer_part = int(record.call_tracker_related_id.replace('NewId_', ''))
                    record.call_tracker_related_id_fix = integer_part
                except ValueError:
                    record.call_tracker_related_id_fix = 0
            else:
                record.call_tracker_related_id_fix = 0


class Call_Disposition(models.Model):
    _name = "crm.call.disposition"
    _description = "Call Disposition"
    _rec_name = 'call_disposition_name'
    _order ='call_disposition_order'

    @api.depends('call_disposition_is_connected')
    def _compute_call_disposition_color(self):
        for record in self:
            if record.call_disposition_is_connected != False:
                record.call_disposition_color = 10
            else:
                record.call_disposition_color = 1

    call_disposition_name = fields.Char(string='Name', store=True)
    call_disposition_is_connected = fields.Boolean(string='Calls Connected', store=True)
    call_disposition_color = fields.Integer('Color', compute='_compute_call_disposition_color')
    call_disposition_order = fields.Integer('Disposition Order', store=True)


class Calls_Connected(models.Model):
    _name = "crm.call.connected"
    _description = 'Is Call Connected?'
    _rec_name = 'option'

    option = fields.Char(string='Name', store=True)
    crm_lead_reference = fields.Many2one('crm.lead', store=True, string='CRM Lead Reference')
...
class LogModel(models.Model):
    ...
    x_log_name = fields.Char('Logs', store=True)
    x_log_record_name = fields.Char('Record Name', store=True)
    x_log_old_value = fields.Char('Old Value', store=True)
    x_log_new_value = fields.Char('New Value', store=True)
    x_log_date = fields.Datetime('Date', store=True)
    x_log_user_id = fields.Many2one('res.users', 'User', store=True)
    x_log_value = fields.Char('Related Logs ID', store=True)
    x_log_document_model = fields.Char('Model', store=True)
    x_log_field_change = fields.Char('Field Change', store=True)
    x_log_document_id = fields.Integer('Related Document ID', compute='_compute_x_log_document_id', store=True)

    @api.depends('x_log_value')
    def _compute_x_log_document_id(self):
        for record in self:
            if record.x_log_value and record.x_log_value.startswith('NewId_'):
                try:
                    integer_part = int(record.x_log_value.replace('NewId_', ''))
                    record.x_log_document_id = integer_part
                except ValueError:
                    record.x_log_document_id = 0
            else:
                record.x_log_document_id = 0
...
class TagIT(models.Model):
    _name = "crm.tag.it"
    _description = "CRM Tag IT"

    def _get_default_color(self):
        return randint(1, 11)

    name = fields.Char('Tag Name', required=True, translate=True, store=True)
